...
\end{eqnarray}
\begin{izrek}[Tensor series]\label{izr:e^d}
...
\begin{definicija}[General tensor network]
...
\end{eqnarray}
is the transformation of a program $P=P_n\circ\cdots\circ P_0\in\dP_0$ to a general tensor network, with $e^\D_N P_i$ being applied at a specific point $\vv_{i}=P_{i-1}(\vv_{i-1})\in \VV$. When the choice of $P\in\dP_0$ and $\mathbf{v}_0\in\VV$ is arbitrary, we denote it simply $\NN\in\dP_0$ for brevity.
\end{izrek}
\begin{proof}
...
Every general tensor network is a \emph{Neural Tensor series} of some program $P\in\dP_0$, for some  set of activation functions $\Phi=\{\Phi_k\in\dP_0:\VV\to\VV;\quad 0\le k\le n\}$. This establishes an equivalence between general tensor networks and programming spaces.
\begin{equation}
\NN=\dP
\end{equation}
\end{corollary}
