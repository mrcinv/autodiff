...
Let $\dP_0$ be a differentiable programming space and let $\dP_n<\F_n$ be the
space spanned by $\DD^n\dP_0$ over $K$. We call $\dP_n$ differentiable
...
By Corollary $\ref{izr:dP}$ we may represent calculation of derivatives of the
...
which enables efficient implementation.
 ...
 \begin{proof}
 It is trivial to show that $\frac{d^n}{dh^n}\text{(LHS)}|_{h=0}=\frac{d^n}{dh^n}\text{(RHS)}|_{h=0}$. Then $\text{LHS}$ and $\text{RHS}$ as functions
 of $h$ have coinciding Taylor series and are therefore equal.\\
  $\implies$
