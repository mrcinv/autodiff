...
A \emph{programming space} is a subspace of the function space of maps on a vector
space serving as a \emph{virtual memory} space. An operator of
differentiation on programming spaces is constructed by extending the virtual memory to a
...
\keywords{
  ...
  \acknowledgement{
  First author extends his gratitude to Jure Kališnik for proofreading the proofs, and Borut Robič for general guidance through the academia.
  }
...
State of the art methods for analyzing neural networks \cite{DeepDream} are generalized  to programming spaces in Section \ref{sec:propertyMeasure}, providing a framework for analysis of machine learning architectures and other virtual constructs, as actions on the virtual space. 
...
\end{equation}
We extend the operation of contraction to multi-tensors on $\VV\otimes T(\VV^*)$.
If we write an element $\bfm\in\VV\otimes T(\VV^*)$ as
$\bfM=\bfm_0+\bfm_1+\ldots+\bfm_n$, where $\bfm_k\in\VV\otimes\left( \VV^*
\right)^{\otimes k}$, the contraction by a vector $\vv\in \VV$ equals
\begin{equation}
  \label{eq:multi_contraction}
  \bfM\cdot\vv = \bfm_0+\bfm_q\cdot\vv+\ldots+\bfm_n\cdot \vv.
\end{equation}
For multi-tensor $\bfM$ we can define a polynomial map $\bfM:\vv\mapsto \bfm^n$ using Horner's algorithm
\begin{eqnarray}\label{eq:Horner}
  \label{eq:horner}
  \bfm^0&=&\bfm_n\\
  \bfm^k&=&\bfm_{n-k}+\bfm^{k-1}\cdot\vv
\end{eqnarray}
...
Let $W$ be an element of the virtual memory $\VV\otimes T_n(\VV^*)$ in an analytic machine of Definition \ref{def:analyticMachine}. We define the application of $\vv\in \VV$ by
\begin{equation}
W(\vv)=\sum\limits_{i=0}^n(w_i\in \VV\otimes \VV^{*\otimes i})(\vv^{\otimes i})\in \VV
\end{equation}
where each contraction $(w_i\in \VV\otimes \VV^{*\otimes i})(\vv^{\otimes i})$ is defined by the programming algebra of Theorem \ref{izr:alg} through expression \eqref{eq:Horner}.

\begin{definicija}
A general tensor network $\NN$ is a sequence of maps $L_i:\VV\to
\VV$ called layers, defined by the equation
\begin{equation}
L_{i+1}=\Phi_i\circ\mathbf{W}_i\circ L_i
\end{equation}
where
$\mathbf{W}_i\in \bigoplus\limits_{k=0}^n\VV\otimes \VV^{*k\otimes}$ are the
weights (and the bias) and
$\Phi_i\in\dP_0$ is the activation function.
If we look at a general tensor network with $n$ layers as a map, we denote
\begin{equation}
\NN=L_n\in\dP_0.
\end{equation}
\end{definicija}
...
$$\mathbf{l}_{i+1}=\Phi_i(\mathbf{b}_i+\mathbf{W}_i\cdot\mathbf{l_i})$$
for input $\mathbf{l}_i\in\VV$ and output $\mathbf{l}_{i+1}\in\VV$ at each layer
is a tensor network with $\mathbf{W}=\mathbf{b}_i+\mathbf{W}_i\in \VV\oplus \VV\otimes \VV^*$.
\end{trditev}
...
This offers a broad generalization of a concept of a neural network, with a rich theory of operational calculus at its disposal for the study and training of such objects, as we demonstrate in Section \ref{sec:trainNet}.
...
The transformed program \eqref{eq:PGN1} equals the original program by Theorem \ref{izr:e^d} and Corollary \ref{izr:komp_homo}. But in practice, we are always working with a finite virtual memory $\VV\otimes T_n(\VV^*)$ and the equality becomes an approximation. Thus we treat the transformation of the original program, as the initialization of the weights (and the bias) of a general tensor network to be trained.  This motivates modernizing the established Taylor series.
\begin{izrek}[Neural Tensor series]\label{izr:transToTensorNet}
Let $\Phi=\{\Phi_k\in\dP_0:\VV\to\VV;\quad 0\le k\le n\}$ be the set of activation functions. Then the transformation
\begin{equation}
\mathcal{N}:\dP\times\VV\to\dP
\end{equation}
\begin{eqnarray}
\mathcal{N}\vert_{\mathbf{v}_0}P=\Phi_n\circ e^\D_N\vert_{\mathbf{v}_n}P_n\circ\cdots\circ\Phi_0\circ e^\D_N\vert_{\mathbf{v}_0}P_0
\end{eqnarray}
is the transformation of a program $P=P_n\circ\cdots\circ P_0\in\dP_0$ to a general tensor network, with $e^\D P_i$ being applied at a specific point $\vv_{i}=P_{i-1}(\vv_{i-1})\in \VV$. When the choice of $P\in\dP_0$ and $\mathbf{v}_0\in\VV$ is arbitrary, we denote it simply $\NN\in\dP_0$ for brevity
\end{izrek}
\begin{proof}
...
$$\implies$$
$$\mathcal{N}\vert_{\mathbf{v}_0}P=L_{n+1}\in\dP_0$$
\end{proof}

\begin{corollary}[Neural-Program equivalence]\label{cor:N=P}
Every tensor network can be derived from some program $P\in\dP_0$ and activations $\Phi=\{\Phi_k\in\dP_0:\VV\to\VV;\quad 0\le k\le n\}$. This establishes an equivalence between tensor networks and general programs $\in\dP$.
\begin{equation}
\NN=\dP
\end{equation}
\end{corollary}
...
Corollary \ref{cor:N=P} has direct implications to the study and understanding of \emph{differentiable neural computers} \cite{netRam} through operational calculus.

Theorem \ref{izr:transToTensorNet} has wide applications to different fields. When the original program is a sub-optimal algorithm providing an approximate solution, as it is more often than not in practice, we apply the transformation of Theorem \ref{izr:transToTensorNet} to it. The derived \emph{Neural Tensor series} serves as a great initialization point of the general tensor network to be trained. This may prove fruitful, with the trained network performing better than the original program, as currently neural networks give best results to many problems.
...
appropriate derivative, as it measures the rate of change. A measure easily replaceable by a more sophisticated one, as made available by the theory.
   ...
   The derived procedures $P$, are a broad generalization of special cases such as Google's Deep Dream project \cite{DeepDream}, where the program $T\in\dP_0$ represents a neural network, with the resulting $\vv\in X_j$ being an altered image.
