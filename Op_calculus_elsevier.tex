...
derivatives $\D^k P$ can be also seen as mapping $\VV$ to $\VV_\infty$ by
...
T(\VV^*)\}$, since $\VV$ is naturally embedded into $ \VV\otimes T(\VV^*)$. The Fréchet derivative defines an operator on the space of smooth maps $\VV\to \VV\otimes
 ...
 $P:\VV\to \VV$ by operator $\D$ is the first derivative, while the higher order
...
We seek to generalize notions of differentiable programing space to spaces
of higher order differentiable programs.

\begin{definicija}\label{def:P_n}
Let $\dP_0$ be a differentiable programming space. The space $\dP_n<\F_n$
spanned by $\DD^n\dP_0$ over $K$, where $\DD^n=\{\partial^k;\quad 0\le k\le n\}$, is called a differentiable
programming space of order $n$.
 \end{definicija}
...
which enables efficient implementation.
 ...
 of $h$ have coinciding Taylor series and are therefore equal.\\
  ...
  \lim\limits_{\lVert h\rVert\to 0}(\frac{d}{dh})^ne^{h\D}(p_1\cdot p_2)=\lim\limits_{\lVert h\rVert\to 0}\D^ne^{h\D}(p_1\cdot p_2)=\D^n(p_1\cdot p_2)
  \end{equation}
  $\impliedby$
...
\begin{proof}
 ...
 \lim\limits_{\lVert h\rVert\to 0}(\frac{d}{dh})^n\exp(\D_fe^{h\D_g})=\sum\limits_{\lambda(n)}\prod\limits_{k\cdot l\in\lambda}\Big(\frac{\D_f\D_g^l(g)}{l!}\Big)^k\frac{n!}{k!}\Big(\Big(e^{\D_f}\Big)f\Big)
 \end{equation}
 taking into consideration the fact that $e^{\D_f}(f)$ evaluated at a point $v\in \VV$ is the same as evaluating $f$ at $v$, the expression \eqref{eq:dComposite} equals \eqref{eq:kompproof1} by Faà di Bruno's formula.
   ...
   \lim\limits_{\lVert h\rVert\to 0}(\frac{d}{dh})^n\exp(\D_fe^{h\D_g})=\sum\limits_{\lambda(n)}\prod\limits_{k\cdot l\in\lambda}\Big(\frac{\D_f\D_g^l(g(v))}{l!}\Big)^k\frac{n!}{k!}\Big(f(g(v))\Big)
   \end{equation}
 \end{proof}       
...
\begin{figure}[!h]
\centering
\begin{tikzpicture}[scale=0.6, every node/.style={scale=0.6},node distance = 2cm, auto]
    \node [block] (p1) {$P_1$};
