...
\end{equation}
and is extended by linearity to spaces $\VV\otimes \left(\VV^*\right)^{\otimes n}$.
 If we write an element $\bfM\in\VV\otimes T_n(\VV^*)$ as
$\bfM=\bfm_0+\bfm_1+\ldots+\bfm_n$, where $\bfm_k\in\VV\otimes\left( \VV^*
\right)^{\otimes k}$, the contraction by a vector $\vv\in \VV$ yields an element
of $\VV\otimes T_{n-1}(\VV^*)$\footnote{note that the component $\bfm_0\in\VV$ 
  can not be contracted by the vector $\vv$. The map given by contraction by a
  fixed multi-tensor can be seen as a generalzation of the affine map, where the
  term $\bfm_0$ accounts for translation.} 
\begin{equation}
  \label{eq:multi_contraction}
  \bfM\cdot\vv = \bfm_0+\bfm_1\cdot \vv+\ldots+\bfm_n\cdot \vv.
\end{equation}
For a multi-tensor $\bfM$ we can define a polynomial map denoted by the same letter $\bfM(\vv)$ using Horner's algorithm
\begin{eqnarray}
  \label{eq:horner}
  \bfm^0&=&\bfm_n\\
...
Corollary \ref{cor:N=P} has direct implications to the study and understanding of \emph{differentiable neural computers} \cite{netRam}. As each general tensor network is a Neural tensor series of some program, it provides an elegant way of expressing neural computations through operational calculus. This equivalence between neural processes and programming spaces reveals new directions to explore in search for their interpretation.

Theorem \ref{izr:transToTensorNet} has wide applications to different fields. When the original program is a sub-optimal algorithm providing an approximate solution, as it is more often than not in practice, we apply the transformation of Theorem \ref{izr:transToTensorNet} to it. The derived \emph{Neural Tensor series} serves as a great initialization point of the general tensor network to be trained. This can be seen as a process of \emph{boosting} \cite{boosting}, converting a weaker learner to a strong one, which proves fruitful, as currently neural networks give best results to many problems.
