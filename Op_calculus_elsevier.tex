...
\end{equation}
where each contraction $(w_i\in \VV\otimes \VV^{*\otimes i})(\vv^{\otimes i})$ is defined by the programming algebra of Theorem \ref{izr:alg}.
...
If we look at a general tensor network with $n$ layers as a map, we might also denote
\begin{equation}
\NN=L_n.
\end{equation}
\end{definicija}
...
\end{eqnarray}
is the transformation of a program to a general tensor network, with $e^\D P_i$ being applied at a specific point $\vv_{i}=P_{i-1}(\vv_{i-1})\in \VV$.
\end{izrek}
\begin{proof}
